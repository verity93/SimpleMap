...
namespace DotSpatial.Data
{
    ...
    internal class BinaryRaster<T> : Raster<T>
        where T : struct, IEquatable<T>, IComparable<T>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BinaryRaster{T}"/> class.
        /// This is a completely empty raster that can be custom configured.
        /// </summary>
        public BinaryRaster()
        {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BinaryRaster{T}"/> class with the specified rows and columns.
        ...
        {
            if (File.Exists(fileName)) Filename = fileName;
            IsInRam = inRam && numColumns * numRows < 64000000;
            NumRows = numRows;
        ...
        /// <summary>
        /// Gets the size of T in bytes. This should be overridden, but exists as a "just-in-case" implementation that works for structs,
        /// but definitely won't work correctly for objects.
        /// </summary>
        public override int ByteSize => Marshal.SizeOf(typeof(T));
        ...
        /// <summary>
        /// Gets the number of bands. This is always 1.
        /// </summary>
        public override int NumBands => 1;
        ...
        {
            if (fileName == Filename) throw new ArgumentException(DataStrings.CannotCopyToSelf_S.Replace("%S", fileName));
            if (File.Exists(fileName))
            {
                if (OnFileExists(fileName)) return; // The copy event was cancelled
        ...
        /// </summary>
        /// <param name="fileName">Name of the file the window content will be copied to.</param>
        ...
        public new IRaster GetWindow(int startRow, int endRow, int startColumn, int endColumn, bool inRam)
        {
            int numCols = endColumn - startColumn + 1;
            int numRows = endRow - startRow + 1;
            var result = new BinaryRaster<T> { Filename = Filename, Projection = Projection, DataType = typeof(int), NumRows = endRow - startRow + 1, NumColumns = endColumn - startColumn + 1, NumRowsInFile = NumRowsInFile, NumColumnsInFile = NumColumnsInFile, NoDataValue = NoDataValue, StartColumn = startColumn + StartColumn, StartRow = startRow + StartRow, EndColumn = endColumn + StartColumn, EndRow = EndRow + StartRow };
        ...
        public virtual void OpenWindow(string fileName, int startRow, int endRow, int startColumn, int endColumn, bool inRam)
        {
            Filename = fileName;
            ReadHeader(fileName);
        ...
        public void PasteRaster(Raster<T> sourceRaster, int startRow, int startColumn)
        {
            ...
            if (sourceRaster.CellWidth != CellWidth || sourceRaster.CellHeight != CellHeight) throw new ArgumentException(DataStrings.RastersNeedSameCellSize);
        ...
        public void ReadHeader(string fileName)
        {
            ...
            {
                throw new ArgumentException(DataStrings.ArgumentOfWrongType_S1_S2.Replace("%S1", fileName).Replace("%S2", "BinaryShortRaster"));
            }
        ...
        public void Write(string fileName)
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                if (fi.Length == HeaderSize + ByteSize * NumColumnsInFile * NumRowsInFile)
                {
                    ...
                    fs = new FileStream(fileName, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None, ByteSize * NumColumns);
                    fs.Seek(HeaderSize, SeekOrigin.Begin);
                ...
                Debug.WriteLine("The size of the file was " + fi.Length + " which didn't match the expected " + HeaderSize + ByteSize * NumColumnsInFile * NumRowsInFile);
                File.Delete(fileName);
            }
            ...
            for (int row = 0; row < NumRowsInFile; row++)
            {
                for (int col = 0; col < NumColumnsInFile; col++)
                {
                    if (row < StartRow || row > EndRow || col < StartColumn || col > EndColumn) bw.Write(Convert.ToInt32(noDataValue));
                    else bw.Write(Data[row - StartRow][col - StartColumn]);
                }
        ...
        private TT ConvertTo<TT>(double value)
        {
            // we don't consider whether the current culture might cause an incorrect conversion.
            object ret = null;

            if (typeof(TT) == typeof(byte)) ret = Convert.ToByte(value);
            else if (typeof(TT) == typeof(double)) ret = Convert.ToDouble(value);
            else if (typeof(TT) == typeof(decimal)) ret = Convert.ToDecimal(value);
            else if (typeof(TT) == typeof(short)) ret = Convert.ToInt16(value);
            else if (typeof(TT) == typeof(int)) ret = Convert.ToInt32(value);
            else if (typeof(TT) == typeof(long)) ret = Convert.ToInt64(value);
            else if (typeof(TT) == typeof(float)) ret = Convert.ToSingle(value);
            else if (typeof(TT) == typeof(bool)) ret = Convert.ToBoolean(value);

            if (ret == null) throw new NotSupportedException("Unable to convert type - " + typeof(TT));
